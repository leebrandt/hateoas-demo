(function(){
	...
	'use strict';
	//utility functions for this provider/resource
	...
	function hyperResource($window, $http, $q){
		function resourceFactory(path){
			...
			Resource.prototype.Get = function(params){
				this.Steps = buildHypermediaSteps(path);
				...
				// kick this thing off
				return getLink(this.Steps.slice(0), {href:ApiRoot});
			};
				...
				return self.Get().then(
					function(result){
						...
						if(action){
							...
							console.error('No action found with the name \'' + name + '\'');
						}
					});
			};
